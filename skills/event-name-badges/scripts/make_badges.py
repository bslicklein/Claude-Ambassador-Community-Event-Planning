...
STOCKS = {
    ...
    "cardstock-vert": dict(
        label="Plain 65-80lb cardstock, VERTICAL 3\" x 4\" with crop marks, 4/sheet "
              "(fits 3x4 portrait lanyard sleeves, e.g. Fushing L / Avery 5392 rotated)",
        cell_w=3.0, cell_h=4.0, cols=2, rows=2,
        margin_top=1.5, margin_side=1.25, gutter_x=0.0, gutter_y=0.0,
        crop_marks=True,
        verified=None,
    ),
}
...
def build(args):
    ...
    # Portrait cells (taller than wide) use a centered lanyard layout:
    # spark on top, name in the middle, event + city stacked at the bottom.
    portrait = stock["cell_h"] > stock["cell_w"]
    vspark = f'<span class="spark">{spark}</span>' if spark else ""

    def named(p):
        first = html.escape(p["first"])
        last = f'<div class="last">{html.escape(p["last"])}</div>' if p["last"] else ""
        if portrait:
            return (f'<div class="vtop">{vspark}</div>'
                    f'<div class="mid vmid"><div class="first {name_class(p["first"])}">{first}</div>{last}</div>'
                    f'<div class="vbot"><div class="label">{ev}</div>'
                    f'<div class="loc">{loc}</div></div>')
    ...
    def blank():
        if portrait:
            return (f'<div class="vtop">{vspark}</div>'
                    f'<div class="mid vmid"><div class="writeline"></div>'
...
"""
    if portrait:
        css += f"""
.vtop{{display:flex;justify-content:center;padding-top:.08in}}
.vtop .spark{{width:.78in;height:.78in;margin:0}}
.vmid{{align-items:center;text-align:center}}
.vmid .writeline{{width:78%;margin:0 auto .06in}}
.vbot{{text-align:center}}
.vbot .label{{display:block;font-size:{pt(15)}pt}}
.vbot .loc{{display:block;margin-top:.05in}}
.first.xl{{font-size:{pt(46)}pt}}.first.lg{{font-size:{pt(38)}pt}}
.first.md{{font-size:{pt(31)}pt}}.first.sm{{font-size:{pt(25)}pt}}
"""
